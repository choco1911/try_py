...
def go_single(t_domain_ip, cmd_domains=None, port=8274):
    domain_list = []
    for domain in cmd_domains:
        domain = toFqdn(cutDomain(domain))
        ipaddr = resolvDomain(domain) 
        if checkService(domain, port) :
            for doms in t_domain_ip:
                if domain != doms[0]:
                    domain_list.append(doms[0])
            print(domain, ipaddr)
